...
function Card({ label, value, sub, color = 'var(--accent)', small = false }) {
  return (
    <div style={{
      ...
      <div style={{
        fontSize: small ? '1.1rem' : 'clamp(1rem, 2.5vw, 1.5rem)',
        fontWeight: 700, color: 'var(--text)', lineHeight: 1.2,
        fontVariantNumeric: 'tabular-nums',
      }}>{value}</div>
      {sub && (
        <div style={{
          fontSize: '0.7rem', color: 'var(--text-3)', marginTop: '0.3rem',
        }}>{sub}</div>
      )}
    </div>
  )
    ...
    <div className="fade-in">
      ...
      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '1.75rem' }}>
        ...
        <Card label="SEM RETORNO" value={fmtBRL(kpis.semRetornoValor)}
          color="var(--danger)"
          sub={`${fmtInt(kpis.semRetornoCount)} notas verdadeiramente pendentes`} />
        <Card label="Com Erro" value={fmtBRL(kpis.comErroValor)}
          color="var(--warning)"
          sub={`${fmtInt(kpis.comErroCount)} notas com problema`} />
      </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th className="right">NF Mês Ant.</th>
          ...
          <tbody>
            {summary.map(s => (
              <tr key={s.key}>
                ...
                <td className="right">
                  <span style={{
                    background: s.pendenteCount > 0 ? '#FEF0EE' : '#F0FAF4',
                    color: s.pendenteCount > 0 ? 'var(--danger)' : 'var(--success)',
                    borderRadius: 6, padding: '0.15rem 0.6rem', fontWeight: 600,
                    fontSize: '0.75rem',
                  }}>{s.pendenteCount}</span>
                </td>
